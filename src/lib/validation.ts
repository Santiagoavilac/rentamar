...
export const guestSchema = z.object({
  ...
  email: z.email("El correo no es válido. Tiene que tener el formato nombre@dominio.com").max(160, "El correo es demasiado largo"),
  phone: z.string().trim().min(6, "El teléfono es demasiado corto (mínimo 6 dígitos)").max(30).optional(),
  documentId: z.string().trim().min(4, "El carnet es demasiado corto (mínimo 4 caracteres)").max(40),
  nationality: z.string().trim().min(3, "Ingresá la nacionalidad").max(60),
  city: z.string().trim().min(2, "Ingresá la ciudad").max(80),
});

export const companionSchema = z.object({
  fullName: z.string().trim().min(2, "Ingresá el nombre completo del acompañante").max(120),
  documentId: z.string().trim().min(4, "El carnet es demasiado corto (mínimo 4 caracteres)").max(40),
  phone: z.string().trim().min(6, "El teléfono es demasiado corto (mínimo 6 dígitos)").max(30).optional().or(z.literal("")),
});
...
export const createAffiliateRequestSchema = z
  .object({
    ...
    affiliate: z.object({
      ...
      email: z.email("El correo no es válido. Tiene que tener el formato nombre@dominio.com").max(160).optional().or(z.literal("")),
...
const adminBookingBaseSchema = z.object({
  ...
  guestEmail: z.email("El correo no es válido. Tiene que tener el formato nombre@dominio.com").max(160),
...
export const createUserInputSchema = z.object({
  email: z.email("El correo no es válido. Tiene que tener el formato nombre@dominio.com").max(160),
